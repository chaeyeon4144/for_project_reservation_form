<template>
  ...
  <div class="faq-categories">
    <button
      ...
      :key="id"
      :class="['category-btn', { active: selectedCateroy === category.id }]"
      @click="selectedCateroys(category.id)">
      {{ category.name }}
    </button>
  </div>
  <div class="faq-list">
    <div v-for="faq in filteredFAQs" :key="id" class="faq-item">
      <div class="faq-question" @click="toggleFAQ(faq.id)">
        <h3>{{ faq.question }}</h3>
        <span class="toggole-icon">{{ activeIndex === faq.id ? "▼" : "▶" }}</span>
      </div>
  ...
  <div v-if="filteredFAQs.length === 0" class="no-result">검색 결과 없습니다.</div>
...
const selectedCateroy = ref("all");
function selectedCateroys(categoryId) {
  selectedCateroy.value = categoryId;
}
// 카테고리 목록 정의
const categories = [
  { id: "all", name: "전체" },
...
const filteredFAQs = computed(() => {
  return faqs.filter(
    (faq) =>
      (selectedCateroy.value === "all" || faq.category === selectedCateroy.value) &&
